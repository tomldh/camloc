...
def createData(N):
    ...
    data = Variable(torch.FloatTensor([[0., 0., 0.], [0., 1., 0.], [
                    1., 1., 0.], [1., 0., 0.]]), requires_grad=True)
...
def createRotMat3D(angleX=0, angleY=0, angleZ=0):
    ...
    rotX = Variable(torch.FloatTensor([[1, 0, 0], [0, cos(
        angleX), -sin(angleX)], [0, sin(angleX), cos(angleX)]]), requires_grad=False)

    rotY = Variable(torch.FloatTensor(
        [[cos(angleY), 0, -sin(angleY)], [0, 1, 0], [sin(angleY), 0, cos(angleY)]]), requires_grad=False)

    rotZ = Variable(torch.FloatTensor(
        [[cos(angleZ), -sin(angleZ), 0], [sin(angleZ), cos(angleZ), 0], [0, 0, 1]]), requires_grad=False)
...
if __name__ == '__main__':
    ...
    estRotMat = runKabsch(samples_rot.clone(), samples.clone())
